...
def generate_level(x):
    if x < 0.06380208333333348:
        return 0
    elif 0.06380208333333348 <= x < 0.08680555555555558:
        return 1
    elif 0.08680555555555558 <= x < 0.11111111111111127:
        return 2
    elif 0.11111111111111127 <= x < 0.1440972222222222:
        return 3
    else:
        return 4
...
class Decoder(nn.Module):
    def __init__(self, output_dim, enc_hid_dim, dec_hid_dim, dec_layers,
                 dropout_p, attention, level_output_dim=5):
        super(Decoder, self).__init__()

        self.enc_hid_dim = enc_hid_dim
        self.dec_hid_dim = dec_hid_dim
        self.output_dim = output_dim
        self.level_output_dim = level_output_dim
        self.dec_layers = dec_layers
        self.dropout_p = dropout_p
        ...
        self.out = nn.Linear(self.enc_hid_dim * 2 + self.dec_hid_dim + self.dec_hid_dim + self.level_output_dim,
            self.output_dim)
        self.out_level = nn.Linear(self.enc_hid_dim * 2 + self.dec_hid_dim + self.dec_hid_dim + self.level_output_dim,
                                   self.level_output_dim)
        self.dropout = nn.Dropout(self.dropout_p)

    def forward(self, input, hidden, cell, encoder_outputs, class_probs):
        ...
        #
        # print("Input dec", embedded.shape)
        ...
        output_levels = self.out_level(torch.cat((output, weighted, input_dec, class_probs), dim=1))
        # print(F.softmax(output_levels, 1))
        # print("Lin input", lin_input.shape)
        # print("Input dec", input_dec.shape)
        # print("Output", output.shape)
        # print("Weighted", weighted.shape)
        # print("Class probs", class_probs.shape)
        # print("CONCAT", torch.cat((output, weighted, input_dec, class_probs), dim=1).shape)
        output = self.out(torch.cat((output, weighted, input_dec, class_probs), dim=1))
        return output.squeeze(1), (hidden, cell), a, F.softmax(output_levels, 1).squeeze(1) #.argmax(1)  # Use this to get level.
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5):
        ...
        output_levels = torch.zeros(max_len, batch_size,
                              self.decoder.level_output_dim).to(self.device)
        ...
        output = src[-1, :, 0]
        # print("OUTPUT", output)
        probs = torch.zeros(list(output.shape)[0], 5)
        for i in range(list(output.shape)[0]):
            probs[i, generate_level(output[i])] = 1
        for t in range(0, max_len):

            output, (hidden, cell), attn_weight, output_level = self.decoder(output, hidden, cell, encoder_outputs, probs)
            outputs[t] = output.unsqueeze(1)
            output_levels[t] = output_level
            teacher_force = random.random() < teacher_forcing_ratio

            output = (trg[t].view(-1) if teacher_force else output)
        return outputs, output_levels
