...
def predict_ts(model, X_test, scaler_y, max_gap_size=6, BATCH_SIZE=1,device=device):
    model.eval()
    perm_idx = np.random.permutation(X_test.shape[0])
...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load('checkpoints/checkpoint.pt', map_location='cpu'))

    test_loss, test_mae, test_rmsle, test_rmse = evaluate(
        model, criterion, X_test, y_test, scaler_x, scaler_y)
    ...
    X_test = X_test[130:131,:,:]
    y_test = y_test[130:131,:,:]

    # scalar

    y_test_origin = scaler_y.inverse_transform(
    y_test.reshape(-1, 1))
    X_test_origin = scaler_x.inverse_transform(
    X_test.reshape(-1, 1))
    ...
    outputs_ori, outputs_scal = predict_ts(model, X_test, scaler_y, max_gap_size=6, BATCH_SIZE=1,device=device)

    print('outputs_ori:{}'.format(outputs_ori))
    print('*************')
    print('outputs_scal:{}'.format(outputs_scal))

    X_test_origin = X_test_origin.flatten().tolist()
    y_test_origin = y_test_origin.flatten().tolist()
    outputs_ori = outputs_ori.flatten().tolist()
    outputs_scal = outputs_scal.flatten().tolist()


    plt.figure()
    plt.plot(X_test_origin+outputs_ori, label='Predicted')
    plt.plot(X_test_origin+y_test_origin, label="True")
